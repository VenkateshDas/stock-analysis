...
class IndiaMarketDataAdapter:
    ...
    def fetch_intraday(self, symbol: str, start: date, end: date, interval: str = "1m") -> pd.DataFrame:
...
def default_data_adapter() -> IndiaMarketDataAdapter:
    data_dir = Path(__file__).resolve().parents[3] / "data" / "bot" / "market"
    return IndiaMarketDataAdapter(data_dir=data_dir)
